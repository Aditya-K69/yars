...
function colorToRgb(color: string): [number, number, number] {
  ...
  const matches = computed.match(/\d+(?:\.\d+)?/g);
...
void mainImage(out vec4 C, in vec2 U) {
  ...
  for (int i = 0; i < MAX_STEPS; ++i) {
    ...
    float px = p.x;
    float wob1 = uBend1 + sin(t1 + px * 0.25) * 0.35;
    float wob2 = uBend2 + cos(t2 + px * 0.30) * 0.35;
...
interface AsciiPlasmaWaveProps {
  xOffset?: number;
  yOffset?: number;
  rotationDeg?: number;
  focalLength?: number;
...
export default function AsciiPlasmaWave(props: AsciiPlasmaWaveProps) {
  const {
    xOffset = 0,
    yOffset = 0,
    rotationDeg = 0,
    ...
    bend1 = 1,
    bend2 = 0.5,
    colors = ["var(--primary)", "var(--chart-2)"] as [string, string],
    cellSize = 14,
    cellFill = 0.6,
    asciiChars = " .:-=+*#%@",
  } = props;
  ...
  useEffect(() => {
    ...
    if (!textureRef.current) {
      ...
      (textureRef.current as any).image = canvas;
      (textureRef.current as any).needsUpdate = true;
    }
  ...
  useEffect(() => {
    ...
    const { canvas: atlasCanvas, count: glyphCount } = buildGlyphAtlas(
      asciiChars,
      cellFill,
    );
    ...
    const update = (now: number) => {
      const {
        ...
        bend1: b1 = 1,
        bend2: b2 = 0.5,
        colors: cols = ["var(--primary)", "var(--chart-2)"] as [string, string],
        cellSize: cSize = 14,
      } = propsRef.current;
      ...
      program.uniforms.uColor1.value = colorToRgb(cols[0]);
      program.uniforms.uColor2.value = colorToRgb(cols[1]);
